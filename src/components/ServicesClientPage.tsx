...
export default function ServicesClientPage({ services, categories }: Props) {
  ...
  const downloadServiceReceipt = async (data: any, paymentId?: string) => {
    ...
    try {
      ...
      doc.setFontSize(8);
      doc.text("SERVICE RECEIPT & INVOICE", 52.5, 15, { align: "center" });
      ...
      drawRow("Service", activeSvc.name);
      drawRow("Rate/Price", activeSvc.price);
      if (uploadFile) {
        drawRow("Attachment", uploadFile.name);
      }
      drawRow("Copies", "1");
      ...
      if (isOnline && paymentId) {
        drawRow("Payment ID", paymentId);
      }

      // Warning Footer Banner
      const bannerY = (isOnline && paymentId) ? 134 : 128;
      doc.setFillColor(224, 22, 43);
      doc.rect(10, bannerY, 85, 7, "F");
      doc.setTextColor(255, 255, 255);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(6.5);
      doc.text(isOnline ? "SHOW RECEIPT AT COUNTER TO COLLECT" : "SHOW TRACKING ID AT COUNTER TO PAY & COLLECT", 52.5, bannerY + 4.5, { align: "center" });
          ...
          >
            ...
            {trackingId ? (
              <div className="text-center py-4">
                <CheckCircle className="w-16 h-16 mx-auto text-green-500 mb-4 animate-[scaleUp_0.3s_ease-out]" />
                <h3 className="text-2xl font-black uppercase mb-2">Request Submitted!</h3>
                ...
                {activeSvc.generateReceipt !== false ? (
                  <>
                    ...
                    <div className="bg-bauhaus-yellow border-4 border-bauhaus-black p-5 mb-6">
                      <p className="text-xs font-black uppercase tracking-widest text-gray-600 mb-1">Tracking ID</p>
                      <p className="text-3xl font-mono font-black">{trackingId}</p>
                    </div>
