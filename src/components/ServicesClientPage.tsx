...
export default function ServicesClientPage({ services, categories }: Props) {
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Initialize file storage entry
      ...
      if (!initRes.ok) {
        throw new Error(`Failed to initialize upload for ${fileToUpload.name}`);
      }
      ...
      const uploadChunk = async (chunkIndex: number) => {
        ...
        if (!chunkSuccess) {
          throw new Error(`Failed to upload chunk ${chunkIndex + 1}/${totalChunks} for ${fileToUpload.name}`);
        }
