import { MapPin, Phone, Clock, Mail, ArrowRight, User } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Contact Us & Location | Shree Krishna Telecom Raipur",
  description: "Contact Shree Krishna Telecom near main market Raipur. Find opening hours, contact numbers, email & Google Maps directions for urgent document printing.",
  keywords: [
    ...
    <div>
      ...
      <section className="bg-bauhaus-black text-bauhaus-white py-16 px-6 border-b-4 border-bauhaus-black">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-xl opacity-80 max-w-xl mb-4">
            Walk in, call us, or send a message. We're always happy to help!
          </p>
          <div className="inline-flex items-center gap-2 bg-bauhaus-yellow text-bauhaus-black border-4 border-bauhaus-black px-4 py-2 font-black uppercase text-sm">
            <User className="w-4 h-4 shrink-0" />
            <span>Owner: <span className="font-black">Sandip Mandal</span></span>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 px-6 border-b-4 border-bauhaus-black">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          {[
            {
              icon: User, title: "Owner", color: "bg-bauhaus-yellow text-black",
              lines: ["Sandip Mandal", "Proprietor & Founder", "Shree Krishna Telecom"],
            },
            {
            ...
            {
              icon: Mail, title: "Email", color: "bg-bauhaus-black text-white",
              lines: [settings.email, "", "Reply within 24h"],
            },
            {
              icon: Clock, title: "Hours", color: "bg-bauhaus-red text-white",
              lines: [
                settings.openDays || "Mon - Sun",
                computedStatus.hoursText,
                computedStatus.isOpen ? `🟢 ${computedStatus.statusText}` : `🔴 ${computedStatus.statusText}`
              ],
            },
          ].map((info) => (
            <div key={info.title} className="border-4 border-bauhaus-black overflow-hidden flex flex-col justify-between">
              ...
              <div className="p-5 bg-bauhaus-white flex-1 flex flex-col justify-center">
                {info.lines.map((l, i) => (
                  <p key={i} className={`${i === 0 ? "font-bold text-base" : "text-gray-500"} text-sm leading-relaxed truncate`}>{l || <br />}</p>
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="border-4 border-bauhaus-black bg-bauhaus-black p-8 text-bauhaus-white">
            <div className="text-center">
              ...
              <p className="text-bauhaus-yellow font-bold text-sm mt-1 uppercase">Owner: Sandip Mandal</p>
