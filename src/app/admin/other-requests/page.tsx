...
import { Download, CheckCircle, RefreshCcw, XCircle, Eye, Trash2, Layers, User, FileText, Search, X } from "lucide-react";
...
// Helper to parse service request notes
const parseServiceRequestNotes = (notes: string | null | undefined) => {
  if (!notes) return { serviceName: "Unknown Service", customerName: "Unknown", customerEmail: "", customerPhone: "", clientNotes: "" };
  
  const serviceMatch = notes.match(/\[Service Request:\s*([^\]]+)\]/);
  ...
  if (serviceMatch) clientNotes = clientNotes.replace(serviceMatch[0], "");
  ...
  return {
    serviceName: serviceMatch ? serviceMatch[1].trim() : "Unknown Service",
...
export default function AdminOtherRequests() {
  ...
  const filtered = requests.filter((req) => {
    ...
    const { serviceName, customerName, customerEmail, customerPhone, clientNotes } = parseServiceRequestNotes(req.notes);
  ...
  const renderRequestItem = (req: PrintRequest) => {
    ...
    const { serviceName, customerName, customerEmail, customerPhone, clientNotes } = parseServiceRequestNotes(req.notes);
    const isPlaceholderFile = req.fileName === "request_placeholder.png";
    ...
    return (
      <div key={req.id} className="bg-white border-4 border-bauhaus-black p-5 shadow-[3px_3px_0_0_#1a1a1a]">
        <div className="flex flex-wrap items-center gap-3 mb-3">
          ...
          <span className="px-2 py-0.5 text-xs font-black bg-bauhaus-blue text-white border border-bauhaus-black uppercase flex items-center gap-1">
            <Layers className="w-3.5 h-3.5" /> {serviceName}
          </span>
          ...
          {req.price !== undefined && req.price !== null && (
            <span className="px-2 py-0.5 text-xs font-black bg-bauhaus-red text-white border border-bauhaus-black">
              ₹{req.price}
            </span>
          )}
        ...
        {isPlaceholderFile ? (
          <p className="text-xs text-gray-400 font-bold uppercase mb-2">No attachment uploaded</p>
        ) : (
            ...
            {parseFiles(req.fileUrl, req.fileName).map((file, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-wrap items-center justify-between border border-gray-200 p-2.5 bg-gray-50 gap-2">
                  <div className="flex items-center gap-2 text-sm text-gray-700 font-medium break-all flex-1 min-w-0 pr-3">
        ...
        {clientNotes && (
          <div className="text-xs text-gray-600 bg-gray-50 border-l-4 border-gray-300 p-2.5 my-2 font-medium">
            <span className="font-bold block uppercase text-[10px] text-gray-400 mb-0.5">Customer Message:</span>
            {clientNotes}
          </div>
        )}
  ...
  return (
    <div>
      <div className="flex flex-wrap justify-between items-center mb-6 border-b-4 border-bauhaus-black pb-4 gap-4">
        <h1 className="text-4xl font-black uppercase">Other Requests</h1>
      ...
      <div className="mb-6 relative">
        <div className="relative">
          <input
            ...
            placeholder="Search by Tracking ID, name, email, phone, service..."
